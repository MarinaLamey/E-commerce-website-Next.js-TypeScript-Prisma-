...
export const getSearchResult = async(pageNumber: number | undefined, searchText: string | undefined , sort: string | undefined): Promise<SearchProps> => {
    try{
            const response = await axios.get(`https://e-commerce-website-next-js-type-scr-liard.vercel.app/api/search?q=${searchText}&pageNumber=${pageNumber}&sort=${sort || "default"}`);
